...
func (h *ArmyHandler) HandleTrainTroops(w http.ResponseWriter, r *http.Request) {
	var req TrainTroopsRequestDTO
	...
	if err := h.Service.TrainTroops(r.Context(), userID, req.TroopID, req.Count); err == nil {
		response.Created(w, nil, "troops trained successfully")
	} else {
		switch {
		case errors.Is(err, service.ErrTroopNotFound):
			response.NotFound(r.Context(), w, err)
		case errors.Is(err, service.ErrInsufficientArmyCapacity):
