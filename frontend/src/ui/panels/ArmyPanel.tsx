...
export default function ArmyPanel({ open, onClose }: ArmyPanelProps) {
  ...
  const handleTrain = async (troopId: string) => {
    ...
    if (res.success) {
      setArmy(res.data);
      dispatch({ type: "SET_ARMY", payload: res.data });
                      ...
                      <div className="flex items-center gap-2">
                        <div className="flex items-center gap-1">
                          <button
                            ...
                            className="w-6 h-6 flex items-center justify-center text-xs border border-purgatory-border rounded text-gray-500 hover:text-gray-200 hover:border-gray-600 transition-all"
                          >
                            −
                          </button>
